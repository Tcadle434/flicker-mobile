...
import { View, Text, Image, Pressable, StyleSheet, Dimensions, Alert, Linking } from "react-native";
...
function buildPlan(
	productId: ProductID,
	productsById: Map<ProductID, PaywallProduct>
): PlanViewModel {
...
export default function Step20Paywall({ onNext, mode = "onboarding" }: Props) {
	...
	const handleRestore = useCallback(async () => {
		...
		try {
			...
			Alert.alert(
				"Nothing to Restore",
				result.error ?? "No active subscription found for this Apple ID."
			);
		} finally {
	...
	const heroBadgeText = showTrialMessaging ? selectedPlanData.trialLabel : "RESUME FLICKER PRO";
	...
	return (
		<View
			style={[
				styles.container,
				{ paddingTop: insets.top + 52, paddingBottom: insets.bottom + 8 },
			]}
		>
			{/* ── Hero ── */}
					...
					return (
						...
						>
							...
							<View style={styles.planRight}>
								<Text
									style={[styles.planPrice, selected && styles.planPriceSelected]}
								>
									{plan.price}
								</Text>
			...
			<Animated.View entering={FadeInDown.delay(520).duration(500)} style={styles.ctaWrap}>
				...
				>
					<Text style={styles.ctaText}>{isLoading ? "Starting…" : ctaText}</Text>
				</Pressable>
				<Text style={styles.ctaFineprint}>{fineprint}</Text>
			</Animated.View>
			...
			<Animated.View entering={FadeInDown.delay(600).duration(400)} style={styles.footer}>
				...
				<Pressable
					onPress={() =>
						Linking.openURL(
							"https://fixed-neon-b69.notion.site/Privacy-Policy-3448d68afd9c8083a172ff882420980a"
						)
					}
				>
				...
				<Pressable
					onPress={() =>
						Linking.openURL(
							"https://fixed-neon-b69.notion.site/Terms-of-Service-3448d68afd9c805bbf74c7fe2a01bfb3"
						)
					}
				>
