...
const FEATURES = [
  ...
  'Tracks your calm streak',
];

export default function Step9ResetMode({ onNext }: Props) {
  return (
    <ContentScreen>
      <View style={styles.content}>
        <Animated.View entering={FadeInDown.delay(200).duration(600)} style={styles.iconContainer}>
          <View style={[styles.modeIcon, { backgroundColor: 'rgba(125, 211, 252, 0.15)' }]}>
            <Text style={styles.iconEmoji}>{'\uD83E\uDDD8'}</Text>
          </View>
        </Animated.View>

        <Animated.Text entering={FadeInDown.delay(400).duration(600)} style={[styles.title, { color: '#7DD3FC' }]}>
          Reset Mode
        </Animated.Text>
        <Animated.Text entering={FadeInDown.delay(500).duration(600)} style={styles.tagline}>
          Meditation & breathing for deep calm
        </Animated.Text>
