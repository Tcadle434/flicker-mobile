...
const FEATURES = [
  'Exercise timer with motivating audio',
  'Track movement sessions',
  'Earn rewards for staying active',
  'Build a consistent exercise routine',
];

export default function Step11MoveMode({ onNext }: Props) {
  return (
    <ContentScreen>
      <View style={styles.content}>
        <Animated.View entering={FadeInDown.delay(200).duration(600)} style={styles.iconContainer}>
          <View style={[styles.modeIcon, { backgroundColor: 'rgba(52, 211, 153, 0.15)' }]}>
            <Text style={styles.iconEmoji}>{'\uD83C\uDFC3'}</Text>
          </View>
        </Animated.View>

        <Animated.Text entering={FadeInDown.delay(400).duration(600)} style={[styles.title, { color: '#34D399' }]}>
          Move Mode
        </Animated.Text>
        <Animated.Text entering={FadeInDown.delay(500).duration(600)} style={styles.tagline}>
          Exercise & movement for your body
        </Animated.Text>
